// web/lib/generate.test.ts — ported verbatim (behaviour) from worker/src/lib.test.js as part of
// the VPS migration (planning/07-VPS-MIGRATION.md Phase 0). Same 46 tests; only type-level
// annotations differ from the original.
//
...
//   - the rate-limit counter logic (mocked storage, not the unit under test)
...
function createMockKv(): KvLike & { _store: Map<string, string> } {
...
describe('validateRequestBody', () => {
  ...
  it('rejects a request missing players', () => {
    const { players: _players, ...rest } = valid;
    const result = validateRequestBody(rest);
    expect(result.valid).toBe(false);
  ...
  it('rejects a request missing courts', () => {
    const { courts: _courts, ...rest } = valid;
    const result = validateRequestBody(rest);
    expect(result.valid).toBe(false);
  ...
  it('rejects a request missing duration_minutes', () => {
    const { duration_minutes: _duration, ...rest } = valid;
    const result = validateRequestBody(rest);
    expect(result.valid).toBe(false);
