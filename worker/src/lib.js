// worker/src/lib.js — pure(ish) logic for the AI Drill Builder Worker, kept separate from
// worker/src/index.js so DOMAIN.worker-unit-tests can exercise it directly with `npx vitest run
// worker/` without needing a live Cloudflare runtime.
//
...
export function validateRequestBody(body) {
  ...
  if (!Number.isInteger(body.players) || body.players < 1) {
    errors.push('players is required and must be a positive integer');
  }

  if (!Number.isInteger(body.courts) || body.courts < 1) {
...
export function clampDiagram(diagram) {
  if (!diagram || typeof diagram !== 'object') return diagram;

  const players = Array.isArray(diagram.players)
    ? diagram.players.map((p) => ({ ...p, x: clamp01(p && p.x), y: clamp01(p && p.y) }))
    : diagram.players;

  const arrows = Array.isArray(diagram.arrows)
    ? diagram.arrows.map((a) => ({
        ...a,
        points: Array.isArray(a && a.points)
          ? a.points.map((pt) => [clamp01(pt && pt[0]), clamp01(pt && pt[1])])
          : a && a.points,
      }))
    : diagram.arrows;

  return { ...diagram, players, arrows };
}
 ...
 */
export function drillNameMatchesPlan(drillName, planMarkdown) {
 ...
 * dropped.
 */
export function filterValidDiagrams(drills, planMarkdown) {
...
// -----------------------------------------------------------------------------------------------
// Rate limiting — Cloudflare KV counter, 30 generations/IP/hour (see 03-TECHNICAL-ARCHITECTURE.md)
// -----------------------------------------------------------------------------------------------
...
/**
 * Pure decision function: given the existing KV record (or null) and the current time, decides
 * whether this request is allowed and what the new record should be. No KV access here — this is
 * what makes the rate-limit *logic* unit-testable without a real (or even mocked) KV binding.
 */
export function computeRateLimitDecision(record, now) {
  if (!record || typeof record.count !== 'number' || typeof record.resetAt !== 'number' || now >= record.resetAt) {
    return {
      allowed: true,
 ...
 */
export async function applyRateLimit(kv, ip, now = Date.now()) {
...
export function isAllowedOrigin(origin, extraAllowedOrigins = []) {
