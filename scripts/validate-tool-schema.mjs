#!/usr/bin/env node
// scripts/validate-tool-schema.mjs — DOMAIN.tool-schema-valid
//
// Validates worker/src/schema.js's RETURN_SESSION_PLAN_TOOL — the single source of truth for the
// `return_session_plan` forced tool-use schema — against planning/05-AI-DRILL-BUILDER-PROMPT.md
...
//   3. A known-good sample payload validates; a handful of known-bad payloads each fail for the
//      expected reason — proves the compiled schema actually enforces the constraints, not just
//      that it parses.
//
// This is a structural/schema check (ajv), not a domain formula — see 00-master-plan.md's "Tier:
// lite" note for why this stays out of ground-truth/*.yaml despite the `domain` tag.

import path from 'node:path';
import { fileURLToPath } from 'node:url';
import Ajv from 'ajv';
import { ARROW_TYPES, PLAYER_COLORS, RETURN_SESSION_PLAN_TOOL } from '../worker/src/schema.js';

const REPO_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const SCHEMA_PATH = path.join(REPO_ROOT, 'worker', 'src', 'schema.js');
...
// --- Report -----------------------------------------------------------------------------------
const rel = path.relative(REPO_ROOT, SCHEMA_PATH);
...
console.log(`validate-tool-schema: ${rel} — RETURN_SESSION_PLAN_TOOL matches the spec, schema compiles, and enforces its constraints`);
process.exit(0);
