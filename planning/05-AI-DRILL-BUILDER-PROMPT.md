...
- **Model:** `anthropic/claude-haiku-4.5:exacto` via OpenRouter is the Phase 4 default (see "Provider/model" above). Picked over the alternatives tested — `openai/gpt-5.4-mini:exacto` and `google/gemini-3.1-flash-lite` — because this endpoint lives or dies on the model actually honoring forced tool-use every time, and Exacto routing exists specifically to bias toward providers with strong tool-calling reliability; the ~45¢/month spread between these three candidates at club volume (dozens of generations/month) isn't worth optimizing around. Run the Phase 4 test matrix below against all three before finalizing, using diagram-degrade rate (see "Malformed-diagram fallback" below) as the reliability signal, and swap the `model` string if a cheaper option holds up.
...
- Diagrams: see `06-SVG-DIAGRAM-SYSTEM.md` for the full rendering architecture. The Worker reads `drills[].diagram` from the tool-use response (not regex-extracted — see "Structured output via forced tool-use" above), clamps `x`/`y` to `[0,1]` as a final defensive check, and passes each diagram to the shared `renderCourtDiagram` component, matched to its position in `plan_markdown` by `drill_name`. No image-generation call is involved anywhere in this flow.
